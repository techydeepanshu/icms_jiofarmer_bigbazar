...
const app=express();
const authString=true?"MeCHHkZ9:tdypsA =:lqBZghxJgaVE":"lRRqlkYefuV=:lRRqlkYefuV6jJ==:qzOUsBmZFgMDlwGtrgYypxUz";
app.use(cors());
app.use(bodyParser.json());
app.use(bodyParser.urlencoded({extended:true,useNewUrlParser:true}));
mongoose.connect("mongodb://verveuser:vervebot123@54.234.86.83/vervedb").then(res=>console.log("success connecting to mongo")).catch(err=>console.log(err));
const Schema=mongoose.Schema;
const posSchema=new Schema({SKU:String,UPC:String,ALTUPC1:String,ALTUPC2:String,ItemName:String,Vintage:String,TotalQty:String,Cost:String,PriceA:String,PriceB:String,PriceC:String,Department:String,SALEPRICE:String,SizeName:String,PackName:String,Price:String,STORECODE:String,soldQty:{type:String,default:"0"},wordpressSoldQty:{type:String,default:"0"},invoiceQty:{type:String,default:"0"}});
let pos=mongoose.model("pos",posSchema);
const invoiceSchema=new Schema({Item:String,Description:String,Quantity:String,Price:String,sku:String,category:String,POS:String,Barcode:String,PosSKU:String});
const notFoundSchema=new Schema({Item:String,Description:String,Quantity:String,Price:String,sku:String,Barcode:String,PosSKU:String,InvoiceName:String});
...
app.put("/pos/:upc/inv",(req,res)=>{
  pos.findOne({UPC:req.params.upc},(err,x)=>{
    if(err)res.json("Some error occured");
      ...
      pos.updateOne({UPC:req.params.upc},{invoiceQty:z.toString()},(err,y)=>{
        if(err)res.json("Some error occured");
        else res.json("Product updated successfully");
      });
    }
